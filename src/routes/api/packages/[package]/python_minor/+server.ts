...
export const GET: RequestHandler = async (event) => {
	...
	const rl = await rateLimit(event, "api:python_minor", 300, 3600);
	if (rl.limited) {
		return jsonError(
			event,
			429,
	...
	try {
		const downloads = await getPythonMinorDownloads(
			packageName,
			version || undefined,
		);
		...
		trackApiEvent(
			"api_python_minor",
			`/api/packages/${encodeURIComponent(packageName)}/python_minor`,
			{
				package: packageName,
