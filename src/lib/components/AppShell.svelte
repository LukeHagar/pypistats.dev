...
<div class="min-h-screen">
	<header class="sticky top-0 z-10 backdrop-blur">
		<div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
			<a href="/" class="text-lg font-bold tracking-tight text-[var(--color-surface-50-950)]">
				PyPI Stats
			</a>
			...
			<div class="sm:hidden">
				<Dialog open={mobileOpen} onOpenChange={(d: any) => (mobileOpen = Boolean(d?.open))}>
					<Dialog.Trigger class="btn btn-sm preset-filled" aria-label="Open menu">Menu</Dialog.Trigger>
					...
					<Dialog.Positioner class="fixed inset-0 flex items-start justify-end p-4">
						<Dialog.Content class="card preset-filled w-full max-w-xs p-4 shadow-xl">
							<div class="flex items-center justify-between">
								<div class="text-sm font-semibold text-[var(--color-surface-50-950)]">Menu</div>
								<Dialog.CloseTrigger class="btn btn-sm preset-filled" aria-label="Close menu"
	...
	<footer class="mx-auto mt-16 max-w-7xl px-4 pb-12 sm:px-6 lg:px-8">
		<div class="text-center text-sm text-[var(--color-surface-400-600)]">
